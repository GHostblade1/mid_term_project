...
# 3. 采集的方法
    def crawler(self, url):
        # 将主页的url放入调度器
        self.fs.add_url_to_unvisited(url)
        # 	5. 重复1-4步，直到调度器为空，停止
        while not self.fs.isempty():
            # 	1. 从调度器中获取一个url
            cur_url = self.fs.get_url_from_unvisited()
            # 	2. 发请求，获取响应，抽取新的urls
            text = self.get_page(url=cur_url)
            urls = self.get_urls(text)
            #   3. 将当前url存入调度器
            self.fs.add_url_to_visited(cur_url)
            # 	4. 将urls存入调度器
            for u in urls:
                self.fs.add_url_to_unvisited(u)
            #   5. 抽取数据并入库
