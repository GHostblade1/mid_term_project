...
# 1. 检验ip地址可用性
    def check_ip(self, ip):
        ...
        print(24, ip)
        try:    # 获得目标ip地址
            target_ip = requests.get(check_url, proxies=ip, timeout=3).text
            if target_ip != host_ip:
                print(25, '检测ip', ip, target_ip, '可用')
                return True
        except:
            print(28, '出现异常ip不可用')
        # 出现异常为不可用
...
# 2. 采集ip地址
    def crawl_ip(self, url):
        ...
        #     ip = ips.readlines()
        # print(38, ip)
        # s = random.randrange(len(ip))
        # l_s = ip[s].strip('://')
        # proxy = {l_s[0]: l_s[0] + '://' + l_s[1]}
        # print(40, proxy)
        ...
        for tr in html.xpath('//tr')[1:]:
            ...
            #ip1 = {tds[5].lower(): tds[5].lower()+'://'+tds[0] + ':' + tds[1]}
            ip2 = tds[5].lower()+'://'+tds[0] + ':' + tds[1]
            #print(ip1)
            if self.check_ip(ip2):
                self.db.save(ip2)
                print(56, '存入ip', ip2, '成功')
        self.pagenum += 1
        # except:
        #     print('采集ip失败！')
